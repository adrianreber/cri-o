...
This script tests CRI-O's pod-level checkpoint and restore functionality. It creates a pod with multiple containers, checkpoints it to an OCI image, removes the pod, and then restores it from the checkpoint image.
...
4. **crictl**: Built from cri-tools with pod checkpoint support (`cd ../cri-tools && make crictl`)
